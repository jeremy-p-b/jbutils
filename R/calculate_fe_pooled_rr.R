...
#'
#' @return A numerical vector with estimate, lower bound, and upper bound
#' @export
#'
#' @examples
...
calculate_fe_pooled_rr <- function(s1x1, s1n1, s1x2, s1n2, s2x1, s2n1, s2x2, s2n2, pretty=TRUE) {
  ...
  if (pretty) {
    return(glue("{scales::number(exp(pooled_log_rr), accuracy=0.01)} ({scales::number(exp(lb), accuracy=0.01)}-{scales::number(exp(ub), accuracy=0.01)})"))
  } else {
